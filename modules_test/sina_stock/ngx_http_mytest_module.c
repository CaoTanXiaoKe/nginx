...
ngx_http_mytest_handler(ngx_http_request_t * r)
{
    ...
    */
    ...
    /*
      ...
      似/list=s_sh000001这样的URI，这与在nginx.conf中配置的子请求
      location中的URI是一致的。
    */
    ...
    /*
      调用ngx_http_subrequest创建子请求，它只会返回NGX_OK或者NGX_ERROR。返回NGX_OK时，
      sr就已经是合法的子请求。注意，这里的NGX_HTTP_SUBREQUEST_IN_MEMORY参数将告诉upstream模块
      把上游服务器的响应全部保存在子请求的sr->upstream->buffer内存缓冲区中。
    */
                                                ...
                                                void *data, ngx_int_t rc)
{
    ...
    /*
      注意，上下文是保存在父请求中的，所以要从 pr 中取出请求上下文。
...
mytest_post_handler(ngx_http_request_t * r)
{
    ...
    */
    ...
    int bodylen = output_format.len + myctx->stock[0].len
                  + myctx->stock[1].len + myctx->stock[4].len - 2*3; 	/* 2*3 为 %V 所占字符数。 */
